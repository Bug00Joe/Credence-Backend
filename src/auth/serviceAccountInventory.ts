...
const inventory: ServiceAccountInventoryEntry[] = [
  ...
  },
  {
    ...
    scopes: [ApiScope.OUTBOX_REEIJECT],
    endpoints: [
      {
        path: '/api/admin/outbox/quarantine/:id/reinject',
        method: 'POST',
        requiredScope: ApiScope.OUTBOX_REIJECT,
  ...
  },
  {
    ...
    scopes: [ApiScope.PAYOUTS_WRITE, ApiScope.TRUST_READ],
    endpoints: [
      {
        path: '/api/payouts',
        method: 'POST',
...
export function validateServiceAccountInventory(accounts: ServiceAccountInventoryEntry[]): ServiceAccountInventoryValidationResult {
  ...
  for (const account of accounts) {
    ...
    const declaredScopes = new Set(account.scopes ?? [])
    const usedScopes = new Set<ApiScope>()
    ...
    for (const endpoint of account.endpoints ?? []) {
      ...
      if (!endpoint.requiredScope) {
        ...
        usedScopes.add(endpoint.requiredScope)
        if (!declaredScopes.has(endpoint.requiredScope)) {
          issues.push(`${account.id} endpoint ${endpoint.method} ${endpoint.path} requires undeclared scope ${endpoint.requiredScope}.`)
        }
      }
    }
    ...
    for (const scope of declaredScopes) {
      if (!usedScopes.has(scope)) {
        issues.push(`${account.id} declares scope ${scope} but no endpoint requires it.`)
      }
    }
  }
