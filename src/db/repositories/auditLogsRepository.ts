...
export class PostgresAuditLogsRepository implements AuditLogRepository {
  ...
  async appendBatch(inputs: AuditLogInput[]): Promise<AuditLogEntry[]> {
    ...
    const ctes: string[] = [
      `locked AS (SELECT true AS locked FROM pg_advisory_xact_lock(hashtext('audit_logs_append_lock')))`,
    ]
    ...
    for (let i = 0; i < n; i++) {
      ...
      const prevSrc = i === 0
        ? '(SELECT COALESCE(row_hash, \'GENESIS\') FROM audit_logs WHERE (SELECT locked FROM locked) ORDER BY seq DESC LIMIT 1)'
