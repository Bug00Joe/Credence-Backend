...
// Every /api route below /api/auth is protected.
...
// Emit versioned audit records only for successful, committed mutations.
app.use("/api", (req, res, next) => {
  res.on("finish", () => {
    ...
    try {
      void Promise.resolve(auditLogService.record(auditRecord)).catch((err) => {
        logger.error({ err, auditRecord }, "audit record failed");
      });
    } catch (err) {
      logger.error({ err, auditRecord }, "audit record failed");
    }
...
// The orgId path parameter must match the authenticated tenant.
app.use(
  "/api/orgs/:orgId/policies",
  (req, res, next) => {
    if (req.params.orgId !== req.tenant?.id) {
